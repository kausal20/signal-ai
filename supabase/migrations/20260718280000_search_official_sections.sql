-- signal_search v7 — Official Sources section is domain-based, not event-based.
-- ---------------------------------------------------------------------------
-- v6 put media event coverage (is_official_company_news) in the top section,
-- while the company's own blog/docs often landed in "analysis". v7 fixes that:
--   official   0  is_official_source = true (company's own publisher)
--   analysis   1  primary-subject media coverage + context
--   …          …  review, comparison, tutorial, etc. (unchanged)
--   mentioned  9  non-primary mentions
-- Within the official section, genuine events (is_official_company_news) rank
-- above routine posts; official sources always rank above media in every tier.
-- ---------------------------------------------------------------------------
