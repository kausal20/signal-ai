// signal-ui-v2 · advisor/CompareAiCard.tsx
// ---------------------------------------------------------------------------
// Minimal hero card for Compare AI. Shares layoutId "compare-surface" with the
// Compare overlay so "Compare" morphs the card into the full page.
// ---------------------------------------------------------------------------
...
export function CompareAiCard({ active, onOpen }: { active: boolean; onOpen: () => void }) {
  ...
  return (
    ...
    >
      <fm.div
        layoutId={active ? undefined : "compare-surface"}
        transition={ENTRY}
      ...
      >
        ...
        <div className="relative overflow-hidden rounded-[30px] bg-[#080b08]/95 p-6 backdrop-blur-xl">
          {!reduce && (
            <fm.div
              aria-hidden
              className="pointer-events-none absolute -left-12 -bottom-12 h-44 w-44 rounded-full bg-green/[0.16] blur-3xl"
          ...
          <div className="relative">
            <div className="mb-5 inline-flex items-center gap-1.5 rounded-full border border-green/20 bg-green/[0.08] px-2.5 py-1 text-[11px] font-bold uppercase tracking-[0.1em] text-green">
              <Scale className="h-3.5 w-3.5" /> Compare AI
            </div>

            <h3 className="text-[26px] font-extrabold leading-tight tracking-[-0.03em] text-foreground">
              Find the right model
            </h3>
            <p className="mt-1 text-[13.5px] text-muted-foreground">Compare any two, instantly.</p>

            <div className="mt-6 flex items-center justify-between">
              <div className="flex items-center gap-1.5 text-[12px] text-muted-foreground">
                <Sparkles className="h-4 w-4 text-green" /> 10 models & tools
              </div>
              <fm.button
                type="button"
                onClick={onOpen}
