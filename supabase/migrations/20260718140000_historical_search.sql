-- Historical intelligence search — signal_search returns the COMPLETE archive
-- history for an entity, not just recent/top-N articles.
-- ---------------------------------------------------------------------------
-- Behaviour:
--   • Entity query (the text resolves to a known entity — company/product/model/
--     person/…): return EVERY content_archive article linked to that entity,
--     ordered published_at DESC (newest first). Old launches, funding, product
--     updates, partnerships, acquisitions all included. High cap (500) so a long
--     company history is never truncated to 30, and never empty just because the
--     company hasn't published recently.
--   • Free-text query (no entity match): keep the existing relevance ranking
--     (FTS + fuzzy + freshness) with the normal limit.
-- Same signature + columns (search/index.ts contract unchanged).
-- ---------------------------------------------------------------------------
...
as $$
  ...
  base as (
    select
      ...
      (a.id in (select article_id from entity_hits)) as is_entity_link,
  ...
  from base
  order by
    -- Entity-linked articles first (a company's history is never crowded out).
    is_entity_link desc,
