-- signal_search v5 — Official Company News is driven by the EDITORIAL verdict
-- (is_official_company_news), not entity relevance. Genuine company events only
-- (launch/funding/acquisition/partnership/announcement/exec-interview/major-
-- research/legal-regulatory). Everything else is bucketed into lower sections.
-- No relevance re-weighting; only the section a row lands in changed.
-- ---------------------------------------------------------------------------
-- Entity-query sections (+ their display order):
--   official   0  genuine company events (is_official_company_news = true)
--   analysis   1  primary subject, no concrete event (context/analysis)
--   review     2   comparison 3   tutorial 4   benchmark 5   list 6
--   repo       7  GitHub projects / docs      opinion 8
--   mentioned  9  the company is not the primary subject
-- Order: section rank asc, then published_at desc (newest-first within a
-- section). Free-text queries keep relevance ranking; section = null.
-- ---------------------------------------------------------------------------
